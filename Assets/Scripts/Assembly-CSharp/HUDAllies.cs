...
public class HUDAllies : UIHandlerComponent
{
	private class AllyButton
	{
		...
		public AllyButton(int index, GameObject prefab, Transform trans)
		{
			mIndex = index;
	...
	private List<AllyButton> mCards = new List<AllyButton>(6);
	...
	public bool enabled
	{
		...
		{
			mEnabled = value;
			foreach (AllyButton mCard in mCards)
			{
	...
	public HUDAllies(GameObject uiParent)
	{
		...
		try
		{
			...
			for (int i = 0; i < selectedHelpers.Count; i++)
			{
				...
				text = text + selectedHelpers[i] + "; ";
				mCards.Add(new AllyButton(i, prefab, mAlliesLocations[mCards.Count]));
			}
		}
		catch (Exception)
		{
