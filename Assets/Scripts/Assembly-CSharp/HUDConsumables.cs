...
public class HUDConsumables : UIHandlerComponent
{
	private class Card
	{
		...
		public void Execute()
		{
			...
			else if (Singleton<Profile>.Instance.CurrentStoryWave != 1 || Singleton<Profile>.Instance.HasWaveBeenCompleted(1) || Singleton<Profile>.Instance.IsInMultiplayerWave)
			{
	...
	public HUDConsumables(GameObject uiParent)
	{
		...
		if (Singleton<Profile>.Instance.MultiplayerData.IsMultiplayerGameSessionActive() || wavePlayed != 1 || Singleton<Profile>.Instance.HasWaveBeenCompleted(wavePlayed))
		{
			return;
		}
