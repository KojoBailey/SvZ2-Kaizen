...
public class HUDAbilities : UIHandlerComponent
{
	private class AbilityButton
	{
		...
		public AbilityButton(int index, GameObject prefab, Transform trans, AbilitySchema abData)
		{
			mIndex = index;
		...
		public void Update(bool shouldUpdateExpensiveVisuals)
		{
			if (mCooldownMax >= 0f)
			{
				...
				if (shouldUpdateExpensiveVisuals)
				{
					mMeterRef.Value = Mathf.Clamp(1f - cooldownFactor, 0f, 1f);
				}
			}
			else if (shouldUpdateExpensiveVisuals)
			{
				mMeterRef.Value = 0f;
			}
	...
	private List<AbilityButton> mCards = new List<AbilityButton>(6);
	...
	public bool enabled
	{
		...
		{
			mEnabled = value;
			foreach (AbilityButton mCard in mCards)
			{
	...
	public void Init()
	{
		...
		try
		{
			...
			for (int i = 0; i < mAbilitiesIDs.Count; i++)
			{
				...
				mCards.Add(new AbilityButton(i, prefab, transform, schema));
			}
		}
		catch (Exception)
		{
	...
	public void Update(bool updateExpensiveVisuals)
	{
		...
		if (mQueuedAbilities.Count > 0)
		{
			var nextAbilityInQueue = mAbilitiesIDs[mQueuedAbilities[0]];
			bool wasAbilitySuccessful = WeakGlobalMonoBehavior<InGameImpl>.Instance.hero.DoAbility(nextAbilityInQueue);
			
			if (wasAbilitySuccessful)
			{
				AbilityButton card = mCards[mQueuedAbilities[0]];
	...
	{
		int kAbilityCmdLen = kAbilityCmd.Length;

		if (eventID.Length > kAbilityCmdLen && eventID.Substring(kAbilityCmdLen) == kAbilityCmd)
		{
			TryUsingAbility(int.Parse(eventID.Substring(kAbilityCmdLen)));
			return true;
		}
